...
const ChatWindow = ({ 
  ...
  socket // 🌟 Socket instance passed down from Home.jsx props
}) => {
  ...
  // 🌟 Clean local submit handler wrapper to retain exact UI form state
  const onFormSubmit = async (e) => {
    e.preventDefault();
    if (!typedMessage.trim()) return;

    // Pehle purana default API pipeline run hoga (jo messages update krta ha)
  ...
  return (
    <div className={`flex-1 flex flex-col min-w-0 bg-[#121212] ${view === 'chat' ? 'flex' : 'hidden md:flex'}`}>
              ...
              <div className="flex items-center gap-4 truncate">
                {selectedChat.isSpace ? (
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 border-2 border-[#121212] rounded-full ${
      ...
      {/* Messages View Box (UI Layout Exactly Maintained) */}
                ...
                <div key={msg._id || index} className={`flex space-x-3 md:space-x-4 ${isMe ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  ...
                  <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} flex-1 min-w-0`}>
                    <div className="flex items-center space-x-2 mb-1">
                      <span className="font-bold text-white text-sm">{isMe ? "You" : selectedChat.name}</span>
                      <span className="text-[10px] text-gray-500">
                        {msg.createdAt ? new Date(msg.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : "Just now"}
                      </span>
                    </div>
                    <div className={`border rounded-2xl p-4 shadow-lg max-w-2xl text-white text-sm break-words whitespace-pre-wrap ${
                      isMe ? 'bg-emerald-600/20 border-emerald-500/20' : 'bg-[#1a2b3b] border-blue-900/30'
                    }`}>
                      {msg.content}
                    </div>
                  </div>
      ...
      {/* Input Message Form Panel (UI Unchanged) */}
